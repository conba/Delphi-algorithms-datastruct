...
begin
  ...
  if (PQCount = 0) then
    ...
    for Inx := 1 to pred(PQCount) do
      if (FCompare(FList.List^[Inx], MaxItem) > 0) then
      begin
...
begin
  ...
  Inx := FList.Count - 2;
  while (Inx >= 0) and
        (FCompare(FList.List^[Inx], aItem) > 0) do
  begin
...
begin
  ...
  while (aFromInx > 0) and
        (FCompare(Item, FList.List^[ParentInx]) > 0) do
  begin
...
begin
  ...
  if (Handle^.peInx > 0) then
  begin
    ParentInx := (Handle^.peInx - 1) div 2;
    ParentHandle := PpqexNode(FList[ParentInx]);
    if (FCompare(Handle^.peItem, ParentHandle^.peItem) > 0) then
    begin
      pqBubbleUp(Handle);
      {$IFDEF DebugMode}
      pqVerifyIndirection;
