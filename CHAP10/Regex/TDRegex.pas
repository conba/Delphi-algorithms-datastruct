...
{===Helper routines==================================================}
function SystemUpcase(aCh : Char) : Char; far;
begin
...
begin
  ...
  try
    ...
    if (FPosn^ <> #0) then begin
      ...
      {$ELSE}
      aErrorPos := FPosn - PChar(FRegexStr) + 1;
      {$ENDIF}
    end;
  except
...
{--------}
function TtdRegexEngine.rcParseCCChar : Char;
begin
...
begin
  ...
  if (FPosn^ <> '-') then begin
    if IgnoreCase then
      Include(aClass^, AnsiChar(Upcase(StartChar)))
    else
      Include(aClass^, AnsiChar(StartChar))
  end
  {otherwise it's a real range, so get the character at the end of the
   range; if that's null, there was an error}
    ...
    for Ch := StartChar to EndChar do begin
      Include(aClass^, AnsiChar(Ch));
      if IgnoreCase then
        Include(aClass^, AnsiChar(Upcase(Ch)));
    end;
  end;
  {if we reach here we were successful}
  Result := true;
