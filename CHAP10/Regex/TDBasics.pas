...
type
  ...
  TtdUpcaseFunc = function(aCh: Char): Char;
...
function TDPosCh(aCh : Char; const S : string) : integer;
function TDChExists(aCh : Char; const S : string) : boolean;
function TDIsDigit(aCh : Char) : boolean;
...
function TDCompareNullStr(aData1, aData2 : pointer) : integer;
begin
  Result := StrComp(PChar(aData1), PChar(aData2));
end;

function TDCompareNullStrText(aData1, aData2 : pointer) : integer;
begin
  Result := StrIComp(PChar(aData1), PChar(aData2));
end;

function TDCompareNullStrANSI(aData1, aData2 : pointer) : integer;
begin
  {$IFDEF Delphi1}
  Result := lstrcmp(PChar(aData1), PChar(aData2));
  {$ENDIF}
  {$IFDEF Delphi2Plus}
  ...
  {$ENDIF}
  {$IFDEF Kylix1Plus}
  Result := strcoll(PChar(aData1), PChar(aData2));
  {$ENDIF}
end;
...
begin
  {$IFDEF Delphi1}
  Result := lstrcmpi(PChar(aData1), PChar(aData2));
  {$ENDIF}
  {$IFDEF Delphi2Plus}
